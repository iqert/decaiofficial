...
class Message
{
    ...
    public function readMessage($client_id, $data)
    {
        if (isset($data['record_id']) && $data['record_id']) {
            Db::name('kefu_record')->where('id', $data['record_id'])->update(['status' => 1]);
        }
    }
    ...
    public function userInitialize($client_id, $data)
    {
        ...
        if ($auto_distribution_csr) {
            ...
            if ($csr) {
                $session = Common::distributionCsr($csr['admin_id'] . '||csr', $_SESSION['user_id']);
            }
        }
    ...
    public function chatRecord($client_id, $data)
    {
        ...
        if (!isset($data['session_id'])) {
            Gateway::sendToClient($client_id, json_encode(['msgtype' => 'chat_record', 'code' => 1, 'data' =>
                [
                    'chat_record'  => [],
                    'session_info' => [
                        'nickname' => '无会话',
        ...
        $chat_record_count = Db::name('kefu_record')
    ...
    public function leaveMessage($client_id, $data)
    {
        ...
        if (Db::name('kefu_leave_message')->insert($data)) {
            ...
            if ($new_message_notice) {
                ...
                $markdown = '### 有新的留言 - ' . $user_info['nickname'] . PHP_EOL .
                    '##### 联系姓名:' . $data['name'] . PHP_EOL .
                    '##### 联系方式:' . $data['contact'] . PHP_EOL .
                    '##### 留言内容:' . $data['message'];
    ...
    public function actionSession($client_id, $data)
    {
        ...
        } else if ($data['action'] == 'transfer_done') {
            ...
            if ($session) {
                ...
                if (Gateway::isUidOnline($csr_id)) {
                    ...
                    $message = [
                        ...
                        'session_id'   => $session['id'],
                        'session_user' => $data['session_user'],
                        'last_time'    => Common::formatTime(null),
                        'last_message' => '本会话被客服 ' . $current_csr_info['nickname'] . ' 转移给您',
                        'online'       => 1,
                        'avatar'       => Common::imgSrcFill($session_user_info['avatar']),
        ...
        } else if ($data['action'] == 'edit_nickname') {
    ...
    */
    public function sendMessage($client_id, $data)
    {
        ...
        $res = Common::chatRecord($data['session_id'], $data['message'], $data['message_type']);
    ...
    public function trajectory($client_id, $data)
    {
        ...
        // 是否屏蔽此人
        $user_info['blacklist'] = Db::name('kefu_blacklist')
            ->where('user_id', $user_info['id'])
            ->value('id');

        $trajectory_count = Db::name('kefu_trajectory')
            ->where('user_id', $user_info['id'])
            ->count('id');
        ...
        if (!$trajectory) {
            ...
            if ($data['page'] == 1) {
                $last_message = [
                    'last_time'    => date('H:i:s', $trajectory[0]['createtime']),
        ...
        Gateway::sendToClient($client_id, json_encode(['msgtype' => 'trajectory', 'code' => 1, 'data' =>
            [
    ...
    public function searchUser($client_id, $keywords)
    {
        ...
        foreach ($session as $key => $value) {
            ...
            $session[$key]['last_time']    = Common::formatTime($last_message['createtime']);
            ...
            $session[$key]['avatar']       = Common::imgSrcFill($value['avatar']);
        ...
        foreach ($invitation as $key => $value) {
            ...
            $invitation[$key]['avatar']           = Common::imgSrcFill($value['avatar']);
