...
class Events
{
    ...
    public static function onWebSocketConnect($client_id, $data)
    {
        ...
        // 安全检查
        array_walk_recursive($data, array('addons\kefu\library\Common', 'checkVariable'));

        $now_time                     = time();
        $initialize_data              = [];
        $initialize_data['chat_name'] = Db::name('kefu_config')->where('name', 'chat_name')->value('value');
        $_SESSION['origin']           = $data['server']['HTTP_ORIGIN']; //设置服务器域名
        ...
        if (!isset($data['get']['modulename'])) {
            ...
            common::showMsg($client_id, $initialize_data['chat_name'] . ' 模块未知');
            return;
        }

        if ($data['get']['modulename'] == 'admin' && $data['get']['token']) {
            // 验证管理员身份
            ...
            }, array($client_id, $data['get']['token']));

        } else if ($data['get']['modulename'] != 'admin' && $data['get']['token']) {
            // 验证FA用户身份
            $user_id = Common::checkFaUser($data['get']['token']);
            if ($user_id) {
                // 验证KeFu用户身份
                $token_info = Common::checkKefuUser($data['get']['kefu_user'], $user_id);
                if ($token_info) {
                    // 设置定时器，定时检测用户身份是否过期
        ...
        if ($data['get']['modulename'] != 'admin' && isset($data['get']['kefu_user']) && !$token_info) {
            // 验证KeFu用户身份
            $token_info = Common::checkKefuUser($data['get']['kefu_user'], 0);
        }
        ...
        if ($token_info) {
            ...
            common::showMsg($client_id, $initialize_data['chat_name'] . ' 无法识别链接用户身份，请重新登录！');
            return;
        }
        ...
        if ($data['get']['modulename'] == 'admin') {
            ...
            foreach ($session as $key => $value) {
                ...
                $value['last_time']    = Common::formatTime(isset($last_message['createtime']) ? $last_message['createtime'] : null);
                ...
                $value['avatar']   = Common::imgSrcFill($value['avatar']);
            ...
            if ($reception_count) {
                Db::name('kefu_csr_config')->where('admin_id', $token_info['id'])
                    ->update([
            ...
            foreach ($invitation as $key => $value) {
                ...
                $invitation[$key]['avatar']           = Common::imgSrcFill($invitation[$key]['avatar']);
                ...
                $invitation[$key]['last_time']        = Common::formatTime($value['createtime']);
            }
    ...
    public static function onMessage($client_id, $message)
    {
        ...
        call_user_func_array(array($o, $data['a']), array($client_id, $data['data'])); //调用对象$o($c)里的方法$a
    }
        ...
        if (isset($_SESSION['user_id'])) {
            ...
            if ($user_info['source'] == 'user') {
                ...
                if ($csr_id) {
                    ...
                    $reception_count = Db::name('kefu_csr_config')->where('admin_id', $csr_id)->value('reception_count');
            ...
            Db::name('kefu_trajectory')->insert($trajectory);*/
            ...
            } else if ($user_info['source'] == 'csr' && $user_info['status'] == 3) {
                // 客服保持在线
                ...
                // 客服下线
                Db::name('kefu_csr_config')->where('admin_id', $user_info['id'])
                    ->update([
                        'reception_count' => 0,
                    ]);
            }
