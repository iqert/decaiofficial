...
CREATE TABLE IF NOT EXISTS `__PREFIX__kefu_config` (
  ...
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=22 DEFAULT CHARSET=utf8 COMMENT='客服配置表';
...
INSERT IGNORE INTO `__PREFIX__kefu_config` VALUES ('5', 'new_user_msg', '这是一个欢迎新用户的消息,系统为用户成功分配客服后,自动以该客服身份发送此消息~');
...
CREATE TABLE IF NOT EXISTS `__PREFIX__kefu_csr_config` (
  ...
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=2 DEFAULT CHARSET=utf8 COMMENT='客服代表(csr)配置和状态表';

-- ----------------------------
-- 插入客服代表
-- ----------------------------
BEGIN;
INSERT IGNORE INTO `__PREFIX__kefu_csr_config` VALUES ('1', '1', '8', '1', '1567046865', '0', '0');
COMMIT;
...
CREATE TABLE IF NOT EXISTS `__PREFIX__kefu_leave_message` (
  ...
  PRIMARY KEY (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='用户留言记录';
...
CREATE TABLE IF NOT EXISTS `__PREFIX__kefu_reception_log` (
  ...
  PRIMARY KEY (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='客服接待记录';
...
CREATE TABLE IF NOT EXISTS `__PREFIX__kefu_record` (
  ...
  `message_type` tinyint(1) unsigned NOT NULL DEFAULT '0' COMMENT '消息类型:0=富文本,1=图片,2=文件,3=系统消息',
...
CREATE TABLE IF NOT EXISTS `__PREFIX__kefu_session` (
  ...
  PRIMARY KEY (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='客服会话表';
...
CREATE TABLE IF NOT EXISTS `__PREFIX__kefu_user` (
  ...
  `createtime` int(10) unsigned DEFAULT NULL COMMENT '创建时间',
  PRIMARY KEY (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='KeFu用户表';
...
CREATE TABLE IF NOT EXISTS `__PREFIX__kefu_blacklist` (
  ...
  `createtime` int(10) unsigned DEFAULT NULL COMMENT '创建时间',
  PRIMARY KEY (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='客服黑名单表';
...
CREATE TABLE IF NOT EXISTS `__PREFIX__kefu_fast_reply` (
  ...
  `deletetime` int(10) unsigned DEFAULT NULL COMMENT '删除时间',
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=6 DEFAULT CHARSET=utf8 COMMENT='快捷回复表';
...
CREATE TABLE IF NOT EXISTS `__PREFIX__kefu_kbs` (
  ...
  `deletetime` int(10) unsigned DEFAULT NULL COMMENT '删除时间',
  PRIMARY KEY (`id`)
) ENGINE=MyISAM AUTO_INCREMENT=2 DEFAULT CHARSET=utf8 COMMENT='知识库表';
