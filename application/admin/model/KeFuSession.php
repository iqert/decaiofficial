...
class KeFuSession extends Model
{
    ...
    {
        return \think\Db::name('kefu_record')->where('session_id', $data['id'])->where('message_type', '<>', 3)->where('sender_identity', 1)->count('id');
    }

    public function getCsrMessageCountAttr($value, $data)
    {
        return \think\Db::name('kefu_record')->where('session_id', $data['id'])->where('sender_identity', 0)->count('id');
    }
